...
############# testing the gui - serial connections next #######
...
########## testing user changing position of the pen to draw ##########################
    def draw(self, event= None, ascii_char = None):
        self.prev_x = self.x
        self.prev_y = self.y
        move = None
        ...
        if ascii_char:
            if ascii_char == "Up":
                move = "Up"
            elif ascii_char == "Down":
                move = "Down"
            elif ascii_char == "Left":
                move = "Left"
            elif ascii_char == "Right":
                move = "Right"


        if move == "Up" and self.y > 0:
            self.y -= 1
        elif move == "Down" and self.y < self.h:
            self.y += 1
        elif move == "Left" and self.x > 0:
            self.x -= 1
        elif move == "Right" and self.x < self.w:
            self.x += 1
...
########## saving the drawing as a png ##################################################
    ...
    def read_serial_input(self):
        UP_ARROW = b'\xE0\x48' #trying to see what values may correspond to arrow keys
        DOWN_ARROW = b'\xE0\x50'
        LEFT_ARROW = b'\xE0\x4B'
        RIGHT_ARROW = b'\xE0\x74'
